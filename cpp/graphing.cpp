...
std::string makeGraph(std::string fn){
	...
	if (right-left>5){
		for (i=floor(left)+1;i<floor(right)+1;i++){
			...
			if (top-bottom>5){
				for (ii=floor(bottom)+1;ii<floor(top)+1;ii++){
					if (i%5==0 && ii%5 == 0){
						svg += "<circle cx=\""+convertX(i,left,right)+"\" cy=\""+convertY(ii,bottom,top)+"\" r=\".6\" fill=\"red\"/>";
					}
					else if (i%5==0 || ii%5 == 0){
						svg += "<circle cx=\""+convertX(i,left,right)+"\" cy=\""+convertY(ii,bottom,top)+"\" r=\".4\" fill=\"red\"/>";
					}
					else {
						svg += "<circle cx=\""+convertX(i,left,right)+"\" cy=\""+convertY(ii,bottom,top)+"\" r=\".2\" fill=\"red\"/>";
					}
			...
			if (i%5 == 0 && i != 0){
				svg += "<circle cx=\""+convertX(i,left,right)+"\" cy=\""+convertY(0,bottom,top,3)+"\" r=\"4px\" fill=\"white\"/>";
		...
		for (var ii=floor(bottom)+1;ii<floor(top)+1;ii++){
			if (ii%5 == 0 && ii != 0){
				svg += "<circle cx=\""+convertX(0,left,right,-3)+"\" cy=\""+convertY(ii,bottom,top)+"\" r=\"4px\" fill=\"white\"/>";
